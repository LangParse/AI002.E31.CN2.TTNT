...
def load_and_preprocess(file_path: str, is_train: bool = True):
    ...
    # Prepare final output FIRST (drop target before defining features)
    target_col = "taken"
    if is_train:
    ...
    # Now define feature types on X (after dropping target)
    ...
    exclude_cols = categorical_features + binary_features
    numerical_features = [col for col in X.columns if col not in exclude_cols]
